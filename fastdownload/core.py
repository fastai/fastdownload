...
def download_url(url, dest=None, timeout=None, show_progress=True):
    ...
    urlsave(url, tmp, reporthook=progress if show_progress else None, timeout=timeout)
    return tmp.replace(dest)

# %% ../00_core.ipynb #cc6c19e5
...
class FastDownload:
    ...
    def get(self, url, extract_key='data', force=False):
        ...
        self.arch_path().mkdir(exist_ok=True, parents=True)
        arch = urldest(url, self.arch_path())
        with _locked(arch.with_name(arch.name+'.lock')):
            if not force:
                data = self.data_path(extract_key, arch)
